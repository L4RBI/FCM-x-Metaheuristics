...
# creating an object Agent with deap.
creator.create("Agent", list, fitness=creator.Fitness, velocity=list)
...
# the braquet of the 2.7 equation
def compute_braquet(agent1, agent2, c, f, l, u, lb, ubound, lbound, size):
    distance = []
    ...
    for i in range(size):
        t = numpy.multiply([(((ubound - lbound) * c) / 2),
                            (((u - lb) * c) / 2)], s_thing[i])
    return list(t * rd)
...
def generate(histogram, ubound, lbound, size, u, lb, m):
    agent = creator.Agent(
        [random.uniform(ubound, lbound), random.uniform(u, lb)] for _ in range(size))
    agent.best = agent
...
def updateGrassHopper(agent, Population, c, f, l, best, histogram, u, lb, ubound, lbound, size, m):
    ...
    for p in Population:
        if not numpy.array_equal(p, agent):
            # the sum of the "{ }" part of equation 2.7.
            sigma += numpy.multiply(random.random(), compute_braquet(agent1=agent, agent2=p,
                                                                     c=c, f=f, l=l, lb=lb, u=u, ubound=ubound, lbound=lbound, size=size))
    b = numpy.multiply(best, [[random.random(), random.random()]
                              for _ in range(size)])  # randomizing the second term
    sigma = sigma * [[random.random(), random.random()] for _ in range(size)]
...
# the function used to calculate the fitness set and evaluate.
