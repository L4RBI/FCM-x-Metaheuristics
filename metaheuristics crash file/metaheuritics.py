...
class metaheuristics:
    ...
    def bat(self, N, GEN, bmin = 0, bmax = 255, fmin = 0, fmax = 0.2):
        ...
        toolbox.register("bat", BAT.generate,m = self.m, bmin = bmin, bmax = bmax, histogram = self.histogram, size = self.size, init_R = 0.5, init_A = 0.95)
        ...
        for G in range(GEN): #computing the BA GEN times
            ...
            for bat in Population: #updating the Bat postion one by one and opdating the Global best as well.
                toolbox.update(bat, best = best, G = G, A = mean_A, m = self.m, fmin = fmin, fmax = fmax, histogram = self.histogram)
                A += bat.loudness
            mean_A = A / N
    ...
    def pso(self, N, GEN, pmin = 0, pmax = 255 ,vmin = -100, vmax = 100, constant1 = 2, constant2 =2 , weight = 0.5):
        toolbox = base.Toolbox()
        toolbox.register("particle", PSO.generate, pmin = pmin, pmax = pmax, vmin = vmin, vmax = vmax, dim = self.dim, size = self.size, histogram = self.histogram )
        toolbox.register("population", tools.initRepeat, list, toolbox.particle)
        toolbox.register("update", PSO.updateParticle, constant1 = constant1, constant2 = constant2, weight = weight, vmin = vmin, vmax = vmax, dim = self.dim, data = self.histogram )
        Population = toolbox.population(n = N)
        best = None
        for particule in Population:
            particule.fitness.values = PSO.toolbox.evaluate(data = self.histogram, particule = particule)
        begin_time = datetime.datetime.now()
    ...
    def gao(self, N, GEN, ubound = 0 , lbound = 255, lb = 0):
        u = numpy.amax(self.histogram)
        toolbox = base.Toolbox()
        toolbox.register("agent", GAO.generate, histogram = self.histogram, ubound = ubound, lbound = lbound,u = u, lb = lb, size = self.size, m = self.m) #setting agent as the function that initialize the agent with the function generate and default args.
        ...
        toolbox.register("update", GAO.updateGrassHopper, f = 0.5, l = 1.5, histogram = self.histogram, size = self.size, m = self.m, ubound = ubound, lbound = lbound, u = u, lb = lb) #registering the updateGrassHopper function as update with setting the default values for some of the args.
...
if __name__ == "__main__":
    ...
    x = metaheuristics(path, 3, m = 2)
    z, time = x.pso(N = 100 , GEN = 2000)
    print(z,z.fitness, time)
